...
train<-fread("walmart-train.csv",colClasses = c(Upc='character'))
test<-fread("walmart-test.csv",colClasses = c(Upc='character'))
...
testCluster<-ddply(test,c('VisitNumber'),function(x) c(cluster=generatefineCluster(x)),.progress="text") 
